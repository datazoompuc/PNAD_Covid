...
{synoptline}
{syntab:Inputs}
{synopt:{opt week(numlist)}} semanas da PNAD COVID19 {p_end}
...
{phang} O programa gera uma base para cada semana. Se for o caso, utilize o 
...
{phang} 
{opt week(numlist)} especifica a lista de semanas com os quais o usuário deseja trabalhar. 


{phang} {opt original(str)} indica o caminho da pasta onde estão localizados os arquivos de dados originais. 
Existe um arquivo de microdados para cada semana da pesquisa. Todos eles devem estar posicionados na mesma pasta para que o programa funcione adequadamente. O Portal não disponibiliza os dados originais, que podem ser
